...
const NewTodoForm = ({ noticeSnackbarStatus }) => {
  ...
  const onSubmit = (e) => {
    ...
    form.content.value = '';
    form.content.focus();
    noticeSnackbarStatus.open(`${newTodoId}번 todo 추가되었습니다.`);
  };
...
const TodoListItem = ({ todo, index, openDrawer, removeTodo }) => {
  return (
    <>
      <li key={todo.id}>
        <div className="tw-flex tw-flex-col tw-gap-2 tw-mt-3">
          ...
          <div className="tw-rounded-[10px] tw-shadow tw-flex tw-text-[14px] tw-min-h-[80px]">
            ...
            <Button onClick={() => removeTodo(todo.id)} color="inherit">
              <FaTrash className="tw-text-[#dcdcdc] tw-text-2xl" />
            </Button>
          </div>
        </div>
      </li>
...
function TodoOptionDrawer({ status, noticeSnackbarStatus }) {
  ...
  const removeTodo = () => {
    ...
    status.close();
    noticeSnackbarStatus.open(`${status.todoId}번 todo 삭제되었습니다.`, 'error');
  };
...
const TodoList = ({ noticeSnackbarStatus }) => {
  ...
  return (
    <>
      ...
      />
      <nav>
        전체 글 수 : {todosStatus.todos.length}
        <ul>
          {todosStatus.todos.map((todo, index) => (
            <TodoListItem
              ...
              removeTodo={todosStatus.removeTodo}
            />
          ))}
        </ul>
      </nav>
...
function App() {
  ...
  React.useEffect(() => {
    todosStatus.addTodo('하루 1번\n약 먹기');
    todosStatus.addTodo('산책');
    todosStatus.addTodo('스트레칭');
  }, []);
